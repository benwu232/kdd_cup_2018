...
from lib.data_frame import DataFrame
from lib.tf_base_model import TFBaseModel
...
def tf_standardize(x, axis=1, epsilon=1e-7, with_std=True):
    mean, var = tf.nn.moments(x, [axis], keepdims=True)
    if with_std:
        std = tf.sqrt(var)
        ...
        x_scaled = (x - mean)
        return x_scaled, mean, mean/mean

def tf_unstandardize(x, mean, std=1.0, with_std=True):
    if with_std:
        return x * std + mean
    return x + mean
...
class WaveNetEncDec(TFBaseModel):
    ...
    def __init__(
        ...
        filter_widths=[2 for i in range(8)]*2,
        num_decode_steps=NUM_DECODE_STEPS,
        **kwargs
    ):
        ...
        self.preprocess_type = 'mean'
        self.n_enc_feature = 6
        self.n_dec_feature = 3
        super(WaveNetEncDec, self).__init__(**kwargs)

    def transform(self, x, type='mean'):
        if type == 'mean':
            #return tf.log(x + 1) - tf.expand_dims(self.log_x_encode_mean, 1)
            return x - tf.expand_dims(self.log_x_encode_mean, 1)
        elif type == 'standardize':
            return tf_standardize(x)

    def inverse_transform(self, x, type='mean'):
        if type == 'mean':
            #return tf.exp(x + tf.expand_dims(self.log_x_encode_mean, 1)) - 1
            return x + tf.expand_dims(self.log_x_encode_mean, 1)
        elif type == 'standardize':
            return tf_unstandardize(x, self.avg, self.std)

    def get_input_sequences(self):
        if DBG:
            self.x_encode = tf.placeholder(tf.float32, [self.batch_size, None, 6])
            self.encode_len = tf.placeholder(tf.int32, [self.batch_size])
            self.y_decode = tf.placeholder(tf.float32, [self.batch_size, self.num_decode_steps, 3])
            self.decode_len = tf.placeholder(tf.int32, [self.batch_size])
            self.is_nan_encode = tf.placeholder(tf.float32, [self.batch_size, None, 6])
            self.is_nan_decode = tf.placeholder(tf.float32, [self.batch_size, self.num_decode_steps, 3])
        else:
            self.x_encode = tf.placeholder(tf.float32, [None, None, None])
            self.encode_len = tf.placeholder(tf.int32, [None])
            self.y_decode = tf.placeholder(tf.float32, [None, self.num_decode_steps, None])
            self.decode_len = tf.placeholder(tf.int32, [None])
            self.is_nan_encode = tf.placeholder(tf.float32, [None, None, None])
            self.is_nan_decode = tf.placeholder(tf.float32, [None, self.num_decode_steps, None])

        self.keep_prob = tf.placeholder(tf.float32)
        self.is_training = tf.placeholder(tf.bool)
        decode_idx = tf.tile(tf.expand_dims(tf.range(self.num_decode_steps), 0), (tf.shape(self.y_decode)[0], 1))

        if self.preprocess_type == 'mean':
            self.log_x_encode_mean = sequence_mean(tf.log(self.x_encode + 1), self.encode_len, self.n_enc_feature)
            self.log_x_encode = self.transform(self.x_encode, type='mean')
            #self.x = tf.expand_dims(self.log_x_encode, 2)
            self.x = self.log_x_encode

            self.encode_features = tf.concat([
                self.is_nan_encode,
                tf.tile(tf.expand_dims(self.log_x_encode_mean, 1), (1, tf.shape(self.x_encode)[1], 1)),
            ...
            self.decode_features = tf.concat([
                ...
                self.is_nan_decode, #$$$$ is it right?
                tf.tile(tf.expand_dims(self.log_x_encode_mean[:, :self.n_dec_feature], 1), (1, self.num_decode_steps, 1)),
            ], axis=2)

        elif self.preprocess_type == 'standardize':
            self.x, self.avg, self.std = self.transform(self.x_encode, type=self.preprocess_type)
            self.x = tf.expand_dims(self.x, 2)
            self.encode_features = tf.concat([
                tf.expand_dims(self.is_nan_encode, 2),
                tf.expand_dims(tf.cast(tf.equal(self.x_encode, 0.0), tf.float32), 2),
                tf.tile(tf.reshape(self.avg, (-1, 1, 1)), (1, tf.shape(self.x_encode)[1], 1)),
                tf.tile(tf.reshape(self.std, (-1, 1, 1)), (1, tf.shape(self.x_encode)[1], 1)),
            ...
            self.decode_features = tf.concat([
                tf.one_hot(decode_idx, self.num_decode_steps),
                tf.tile(tf.reshape(self.avg, (-1, 1, 1)), (1, self.num_decode_steps, 1)),
                tf.tile(tf.reshape(self.std, (-1, 1, 1)), (1, self.num_decode_steps, 1)),
    ...
    def encode(self, x, features):
        ...
        y_hat = time_distributed_dense_layer(h, self.n_enc_feature, scope='dense-encode-2')
    ...
    def decode(self, x, conv_inputs, features):
        ...
        # get initial x input, the start positions of a batch
