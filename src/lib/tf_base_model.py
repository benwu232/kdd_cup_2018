...
from lib.tf_utils import shape
...
class TFBaseModel(object):
    ...
    def fit(self):
        with self.session.as_default():
            ...
            train_generator = self.reader.train_bb
            val_generator = self.reader.val_bb
            ...
            while step < self.num_training_steps:
                ...
                val_batch_dict = next(val_generator)
                val_feed_dict = {
                    getattr(self, placeholder_name, None): data
                    for placeholder_name, data in val_batch_dict.items() if hasattr(self, placeholder_name)
                }
                ...
                [labels, preds, val_loss] = self.session.run(
                    fetches=[self.labels, self.preds, self.loss],
                ...
                if step % self.log_interval == 0:
                    print(labels[0], labels[0].mean(), np.median(labels))
                    print(preds[0], preds[0].mean(), np.median(preds[0]))
